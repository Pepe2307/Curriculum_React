...
function ProjectPage() {
    ...
    const filteredArticles = projectsData.filter((project) => {
        const content = `${pick(project.projectName)} ${pick(project.projectDesc)} ${project.tags.join(' ')}`
    ...
    return (
        <div className="projectPage" style={{backgroundColor: theme.secondary}}>
            <Helmet>
                <title>Pablo Paez | {tr.projectsTitle}</title>
            </Helmet>
            <div className="projectPage-header" style={{backgroundColor:theme.primary}}>
                <Link to="/">
                        <AiOutlineHome className={classes.home}/>
                </Link>
                <h1 style={{color: theme.secondary}}>{tr.projectsTitle}</h1>
            </div>
           <div className="projectPage-container">
               <div className="projectPage-search">
                   <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={tr.projectsSearch} className={classes.search} />
               </div>
